...
import { Iitem } from "../project/ProjectItem";

const TestimonialItem = ({ testimonials }: any) => {
  const { carouselFragment, slideToPrevItem, slideToNextItem } =
    useSpringCarousel({
      itemsPerSlide: 3,
      withLoop: true,
      items: testimonials.map((item: Iitem, key: number) => ({
        id: item.key,
        renderItem: (
  ...
  return (
    <div className="testimonial-carousel">
      ...
      <div className="owl-nav">
        <div className="owl-prev" onClick={slideToPrevItem}>
          <FontAwesomeIcon
            className="bi bi-arrow-right"
            icon={faArrowLeftLong}
          />
        </div>
        <div className="owl-next" onClick={slideToNextItem}>
          <FontAwesomeIcon
            className="bi bi-arrow-right"
